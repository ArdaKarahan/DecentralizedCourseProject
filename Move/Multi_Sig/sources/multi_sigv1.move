...
public fun create_wallet(owners: vector<address>, wallet_type: u8, ctx: &mut TxContext) {
    ...
    // ⚠️ v1 VULNERABILITY: No duplicate owner check!
...
public entry fun execute_proposal(
    ...
    proposal: &mut Proposal,
    _clock: &Clock, // Accept Clock parameter for compatibility with v2 tests
    ctx: &mut TxContext,
) {
    assert!(proposal.status == STATUS_PENDING, EProposalNotActive);
    assert!(object::id(wallet) == proposal.wallet_id, 0);

    // ⚠️ v1 VULNERABILITY: No expiry check during execution!
    ...
    // ⚠️ v1 VULNERABILITY: Perform action BEFORE updating status (wrong order)!
    if (proposal.action_type == ACTION_SEND_SUI) {
        // ⚠️ v1 VULNERABILITY: No balance check before coin::take!
        ...
        if (found) {
            // ⚠️ v1 VULNERABILITY: No check to prevent removing last owner!
            vector::remove(&mut wallet.owners, index);
        }
    };

    // Status updated AFTER interaction (bad practice)
...
public fun e_owner_exists(): u64 { EOwnerExists }

public fun e_owner_not_found(): u64 { EOwnerNotFound }
...
/// ⚠️ v1 NOTE: Does not check expiry (vulnerability)
public fun can_execute(wallet: &MultisigWallet, proposal: &Proposal, _clock: &Clock): bool {
    // Must be pending
    if (proposal.status != STATUS_PENDING) {
        return false
    };

    // ⚠️ v1 VULNERABILITY: No expiry check here!
